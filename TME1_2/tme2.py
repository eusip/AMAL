...
def auto_diff(train, test, ctx, linear, criterion, lr):
	...
	for mode in ('batch','mini-batch'):
		...
		train_path = 'auto_diff/train/' + mode
		test_path = 'auto_diff/test/' + mode
...
def regression(train, test, sequential=False):
	...
	for mode in ('batch', 'mini-batch'):
		...
		# train_losses, test_losses = [], []
		mode_train_path = train_path + mode
		mode_test_path = test_path + mode
		for epoch in range(N_EPOCHS):
			# model training
			x_batch, y_batch = batch_size(x_train, y_train, mode)
			...
			# test_losses.append(test_loss)
			writer.add_scalar(mode_train_path, loss, epoch)
			writer.add_scalar(mode_test_path, test_loss, epoch)
		writer.close()
		mode_train_path = train_path
		mode_test_path = test_path
		# plot_losses(train_losses, test_losses, mode)
